...
repo --name=adaptation-common-ginkgo-@RELEASE@ --baseurl=https://releases.jolla.com/releases/@RELEASE@/jolla-hw/adaptation-common/@ARCH@/
repo --name=adaptation-community-common-ginkgo-@RELEASE@ --baseurl=https://repo.sailfishos.org/obs/nemo:/devel:/hw:/common/sailfish_latest_@ARCH@/
...
%packages
patterns-sailfish-device-configuration-ginkgo
%end
...
if [ ! -d ../hybris/hybris-installer/ ]; then
	git clone --depth 1 https://github.com/sailfish-ginkgo/hybris-installer ../hybris/hybris-installer > /dev/null
fi
